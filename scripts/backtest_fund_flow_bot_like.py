...
class BotLikeReplayEngine(BacktestEngine):
        ...
        ctx.update(
            {
                ...
                "cvd_ratio": self._safe_float(row.get("cvd_delta_ratio"), 0.0),
                "cvd_momentum": self._safe_float(row.get("cvd_pressure"), 0.0),
    ...
    def _build_flow_context(self, data: Dict[str, pd.DataFrame], idx_15m: int) -> Optional[Dict[str, Any]]:
        ...
        for key in (
            ...
            flow_context[key] = self._safe_float(tf15.get(key), 0.0)
    ...
    def _clear_position_tracking(self, symbol: str) -> None:
        ...
        for store_name in ("_position_first_seen_ts", "_position_extrema_by_pos", "_pre_risk_exit_streak_by_pos"):
    ...
    def _close_position_bot_like(self, symbol: str, analysis: Dict[str, Any], reason: str) -> None:
        self.close_position(symbol, float(analysis["price"]), analysis["time"], reason)
        ...
        for current_ts in ordered_timeline:
            ...
            for item in close_candidates:
                self._close_position_bot_like(
                    str(item["symbol"]),
                    item["analysis"],
                    reason=f"decision_close:{item['decision'].reason}",
                )
