...
def get_vwap_structure_position_scale(
    ...
    """
    overrides = vwap_structure_overrides if isinstance(vwap_structure_overrides, dict) else {}
    structure_cfg = overrides.get(vwap_state, {})
...
class FundFlowDecisionEngine:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.entry_hard_gate_require_regime_trend = bool(ff.get("entry_hard_gate_require_regime_trend", True))
        ...
        raw_entry_hard_gate_bypass_rules = ff.get("entry_hard_gate_bypass_rules", [])
        self.entry_hard_gate_bypass_rules = (
            [rule for rule in raw_entry_hard_gate_bypass_rules if isinstance(rule, dict)]
            if isinstance(raw_entry_hard_gate_bypass_rules, list)
            else []
        )
        raw_direction_lock_bypass_rules = ff.get("direction_lock_bypass_rules", [])
        self.direction_lock_bypass_rules = (
            [rule for rule in raw_direction_lock_bypass_rules if isinstance(rule, dict)]
            if isinstance(raw_direction_lock_bypass_rules, list)
            else []
        )
        self.entry_hard_gate_flow_min_pass = max(
            1, int(self._to_float(ff.get("entry_hard_gate_flow_min_pass"), 2))
        )
        self.entry_hard_gate_micro_min_pass = max(
            1, int(self._to_float(ff.get("entry_hard_gate_micro_min_pass"), 2))
        )
        self.fast_exit_enabled = bool(ff.get("fast_exit_enabled", False))
        self.fast_exit_direction_lock_bars = max(
            1, int(self._to_float(ff.get("fast_exit_direction_lock_bars"), 2))
        )
        self.fast_exit_trap_high_confidence = max(
            0.0, min(1.0, self._to_float(ff.get("fast_exit_trap_high_confidence"), 0.85))
        )
        ...
        self.vwap_structure_overrides = (
            copy.deepcopy(ff.get("vwap_structure_overrides", {}))
            if isinstance(ff.get("vwap_structure_overrides"), dict)
            else {}
        )
        ...
        self._direction_guide_model = self._normalize_direction_guide_model(
            guide_cfg.get("model", "MACD_BB")
        )
        self._direction_guide_enhanced_fallback_enabled = bool(
            guide_cfg.get("enhanced_fallback_enabled", False)
        )
        ...
        self._direction_guide_trend_relaxed_neutral_zone = max(
            0.0,
            min(0.20, self._to_float(guide_cfg.get("trend_relaxed_neutral_zone"), 0.03)),
        )
        ...
        self._direction_guide_conflict_neutral_zone = max(
            self._direction_guide_neutral_zone,
            min(0.20, self._to_float(guide_cfg.get("conflict_neutral_zone"), 0.03)),
        )
        ...
        self._direction_price_action_override_max_guide_score = max(
            0.0,
            min(0.30, self._to_float(guide_cfg.get("price_action_override_max_guide_score", 0.30), 0.30)),
        )
        ...
        self.macd_mtf_strategy_config = MACDStrategyConfig(
            ...
            weight_4h_enhancement=max(0.0, min(1.0, self._to_float(macd_cfg.get("weight_4h_enhancement"), 0.10))),
        ...
        if self.macd_v2_enabled:
            ...
            pocket_management_cfg = v2_cfg.get("pocket_management_overrides", {}) if isinstance(v2_cfg.get("pocket_management_overrides"), dict) else {}
            ...
            self.macd_v2_config = MACDStrategyV2Config(
                ...
                green_bar_growing_score_min=self._to_float(
                    filter_cfg.get("green_bar_growing_score_min"),
                    0.0,
                ),
                green_bar_growing_score_max=self._to_float(
                    filter_cfg.get("green_bar_growing_score_max"),
                    1.0,
                ),
                ...
                enable_soft_15m_confirmation_when_4h_primary=bool(
                    filter_cfg.get("enable_soft_15m_confirmation_when_4h_primary", True)
                ),
                require_15m_confirmation_gate=bool(
                    filter_cfg.get("require_15m_confirmation_gate", False)
                ),
                soft_15m_entry_score=self._to_float(filter_cfg.get("soft_15m_entry_score"), 0.28),
                soft_15m_neutral_hist_multiple=self._to_float(
                    filter_cfg.get("soft_15m_neutral_hist_multiple"),
                    3.0,
                ),
                soft_15m_max_adverse_hist_multiple=self._to_float(
                    filter_cfg.get("soft_15m_max_adverse_hist_multiple"),
                    8.0,
                ),
                ...
                pocket_management_overrides=copy.deepcopy(pocket_management_cfg),
                ...
                shrink_exit_loss_mitigation_pnl_threshold=self._to_float(
                    stop_cfg.get("shrink_exit_loss_mitigation_pnl_threshold"),
                    -0.005,
                ),
                shrink_exit_loss_mitigation_exit_ratio=min(
                    1.0,
                    max(
                        0.0,
                        self._to_float(stop_cfg.get("shrink_exit_loss_mitigation_exit_ratio"), 0.60),
                    ),
                ),
                shrink_exit_loss_mitigation_ignore_if_pnl_gt=self._to_float(
                    stop_cfg.get("shrink_exit_loss_mitigation_ignore_if_pnl_gt"),
                    0.01,
                ),
            ...
            self.macd_4h_regime_state_cfg = {
                "enabled": bool(regime_state_cfg.get("enabled", False)),
                "dynamic_side_override": bool(regime_state_cfg.get("dynamic_side_override", True)),
                "close_on_reverse": bool(regime_state_cfg.get("close_on_reverse", True)),
                "allow_transition_entry": bool(regime_state_cfg.get("allow_transition_entry", True)),
                "allow_confirmed_entry_without_15m": bool(
                    regime_state_cfg.get("allow_confirmed_entry_without_15m", True)
                ),
                "transition_entry_score": self._to_float(regime_state_cfg.get("transition_entry_score"), 0.72),
                "transition_entry_scale": self._to_float(regime_state_cfg.get("transition_entry_scale"), 0.35),
                "transition_max_leverage": max(
                    1,
                    int(self._to_float(regime_state_cfg.get("transition_max_leverage"), 2)),
                ),
                "transition_min_shrink_pct_long": self._to_float(
                    regime_state_cfg.get("transition_min_shrink_pct_long"),
                    0.55,
                ),
                "transition_min_shrink_pct_short": self._to_float(
                    regime_state_cfg.get("transition_min_shrink_pct_short"),
                    0.55,
                ),
                "transition_min_vwap_score_long": self._to_float(
                    regime_state_cfg.get("transition_min_vwap_score_long"),
                    0.01,
                ),
                "transition_min_vwap_score_short": self._to_float(
                    regime_state_cfg.get("transition_min_vwap_score_short"),
                    0.08,
                ),
                "confirmed_min_4h_bars": max(
                    1,
                    int(self._to_float(regime_state_cfg.get("confirmed_min_4h_bars"), 1)),
                ),
                "confirmed_entry_score": self._to_float(regime_state_cfg.get("confirmed_entry_score"), 0.84),
                "confirmed_entry_scale": self._to_float(regime_state_cfg.get("confirmed_entry_scale"), 0.50),
                "confirmed_max_leverage": max(
                    1,
                    int(self._to_float(regime_state_cfg.get("confirmed_max_leverage"), 3)),
                ),
                "confirmed_min_vwap_score": self._to_float(
                    regime_state_cfg.get("confirmed_min_vwap_score"),
                    0.08,
                ),
                "same_side_bonus": self._to_float(regime_state_cfg.get("same_side_bonus"), 0.03),
            }
    ...
    @staticmethod
    def _normalize_portion_ratio(value: Any, default_ratio: float) -> float:
        if value is None:
        ...
        try:
            if isinstance(value, str):
                raw = value.strip()
                if raw.endswith("%"):
                    return max(0.0, min(1.0, abs(float(raw[:-1])) / 100.0))
                v = float(raw)
            else:
                v = float(value)
        except Exception:
    ...
    def _build_macd_tf_diagnostics(
        ...
        for tf in ("15m", "1h", "4h"):
            ...
            array = raw_array if isinstance(raw_array, (list, tuple)) else []
            hist_present = tf_dict.get("macd_hist") is not None
            prev_present = tf_dict.get("macd_hist_prev") is not None
            ...
            if len(series) > 0:
                ...
                series_source = "array"
                series_len = len(array)
            elif hist_present or prev_present:
    ...
    def _macd_v2_engine_for_symbol(self, symbol: str) -> Tuple[MACDStrategyV2Engine, Dict[str, Any]]:
        ...
        flip_bullish_mode = str(override.get("flip_bullish_mode") or "").strip().lower()
        if flip_bullish_mode == "disabled":
            override_updates["disable_flip_bullish_entries"] = True
            override_updates["force_disable_flip_bullish_entries"] = True
        elif flip_bullish_mode == "trial_only":
            override_updates["disable_flip_bullish_entries"] = True
            override_updates["force_disable_flip_bullish_entries"] = False
            override_updates["disable_flip_bullish_trial_entries"] = False
            override_updates["enable_4h_preflip_trial_entries"] = True
        elif flip_bullish_mode == "enabled_with_strict_threshold":
            override_updates["disable_flip_bullish_entries"] = False
            override_updates["force_disable_flip_bullish_entries"] = False
        elif "disable_flip_bullish" in override:
        ...
        green_bar_growing_mode = str(override.get("green_bar_growing_mode") or "").strip().lower()
        if green_bar_growing_mode == "disabled":
            override_updates["disable_green_bar_growing_entries"] = True
        elif green_bar_growing_mode == "enabled_with_strict_threshold":
            override_updates["disable_green_bar_growing_entries"] = False
        elif green_bar_growing_mode == "trial_only":
            override_updates["disable_green_bar_growing_entries"] = False
        elif "disable_green_bar_growing" in override:
    ...
    @staticmethod
    def _annotate_primary_open_blocker(metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        enriched = dict(metadata or {})
        for key in (
            "symbol_override_block",
            "signal_type_disabled",
            "pocket_override_block",
            "direction_lock_block",
            "entry_hard_gate_block",
        ):
            enriched[key] = bool(enriched.get(key, False))

        blocked_reason = str(enriched.get("blocked_reason") or "").strip().lower()
        reject_code = str(enriched.get("reject_reason_code") or "").strip().lower()
        reject_stage = str(enriched.get("reject_stage") or "").strip().lower()
        symbol_override = enriched.get("symbol_signal_override")
        if isinstance(symbol_override, dict):
            has_symbol_mode = any(
                str(symbol_override.get(name) or "").strip()
                for name in ("flip_bullish_mode", "green_bar_growing_mode")
            )
            has_legacy_disable = any(
                symbol_override.get(name) is True
                for name in ("disable_flip_bullish", "disable_green_bar_growing")
            )
            if has_symbol_mode or has_legacy_disable:
                enriched["symbol_override_source"] = "symbol_signal_override"

        if blocked_reason.startswith("direction_lock_hard_block"):
            enriched["direction_lock_block"] = True
            enriched["primary_open_blocker"] = "direction_lock_block"
            return enriched

        if enriched.get("entry_hard_gate_reason") or enriched.get("entry_hard_gate_failed_bucket"):
            enriched["entry_hard_gate_block"] = True
            enriched["primary_open_blocker"] = "entry_hard_gate_block"
            return enriched
        ...
        if reject_code in {
            ...
            enriched["signal_type_disabled"] = True
            if enriched.get("symbol_override_source") == "symbol_signal_override":
                enriched["symbol_override_block"] = True
                enriched["primary_open_blocker"] = "symbol_override_block"
            else:
                enriched["primary_open_blocker"] = "signal_type_disabled"
            return enriched

        if reject_stage == "pocket_entry_requirements" or reject_code.startswith("pocket_"):
            enriched["pocket_override_block"] = True
            enriched["primary_open_blocker"] = "pocket_override_block"
            return enriched

        if blocked_reason:
            enriched["primary_open_blocker"] = blocked_reason.split(":", 1)[0]
        return enriched
    ...
    @staticmethod
    def _signal_type_matches_side(side: str, signal_type: Any) -> bool:
        raw = str(signal_type or "").strip().lower()
        if side == "long":
            return raw in {"red_bar_growing", "red_bar_shrinking", "flip_bullish"}
        if side == "short":
            return raw in {"green_bar_growing", "green_bar_shrinking", "flip_bearish"}
        return False

    def _build_macd_v2_4h_regime_state(
        self,
        *,
        signal: MACDSignalV2,
        tf_1h: Dict[str, Any],
        ...
        cfg = self.macd_4h_regime_state_cfg or {}
        state: Dict[str, Any] = {
            ...
            "entry_mode": "",
            "entry_scale": 1.0,
            "max_leverage": max(1, self.default_leverage),
            "min_vwap_score": 0.0,
        ...
        direction_1h = self._normalize_signal_side(details.get("direction_1h"))
        direction_4h = self._normalize_signal_side(details.get("direction_4h"))
        signal_type_1h = str(signal.signal_type_1h or details.get("signal_type_1h") or "").strip().lower()
        signal_type_4h = str(details.get("signal_type_4h") or "").strip().lower()
        preflip_direction = self._normalize_signal_side(details.get("macd_4h_preflip_direction"))
        preflip_status = str(details.get("preflip_confirmation_status") or "").strip().lower()
        shrink_pct = max(0.0, self._to_float(details.get("macd_4h_shrink_pct"), 0.0))
        positive_bars = max(0, int(self._to_float(details.get("stable_4h_positive_bars"), 0)))
        negative_bars = max(0, int(self._to_float(details.get("stable_4h_negative_bars"), 0)))
        bull_active = bool(details.get("stable_4h_bull_active", False))
        bear_active = bool(details.get("stable_4h_bear_active", False))
        hist_current = self._to_float(
            details.get("macd_4h_hist_current"),
            self._to_float(tf_4h.get("macd_hist"), 0.0),
        )
        hist_prev = self._to_float(
            details.get("macd_4h_hist_prev"),
            self._to_float(tf_4h.get("macd_hist_prev"), hist_current),
        )
        vwap_score = max(0.0, self._to_float(signal.vwap_score, 0.0))
        confirm_bars = max(1, int(cfg.get("confirmed_min_4h_bars", 1)))
        same_side_bonus = max(0.0, self._to_float(cfg.get("same_side_bonus"), 0.03))
        dynamic_side_override = bool(cfg.get("dynamic_side_override", True))

        long_same_side = direction_1h == "long" or self._signal_type_matches_side("long", signal_type_1h)
        short_same_side = direction_1h == "short" or self._signal_type_matches_side("short", signal_type_1h)
        long_non_opposite = direction_1h != "short" and not self._signal_type_matches_side("short", signal_type_1h)
        short_non_opposite = direction_1h != "long" and not self._signal_type_matches_side("long", signal_type_1h)

        bull_confirmed = (
            (hist_current > 0 and hist_prev <= 0)
            or (
                direction_4h == "long"
                and (
                    signal_type_4h in {"red_bar_growing", "flip_bullish"}
                    or bull_active
                    or positive_bars >= confirm_bars
                )
            )
        )
        bear_confirmed = (
            ...
            or (
                direction_4h == "short"
                and (
                    signal_type_4h in {"green_bar_growing", "flip_bearish"}
                    or bear_active
                    or negative_bars >= confirm_bars
                )
            )
        )
        bull_transition = (
            not bull_confirmed
            and preflip_direction == "long"
            and preflip_status == "aligned"
            and shrink_pct >= self._to_float(cfg.get("transition_min_shrink_pct_long"), 0.55)
            and long_non_opposite
        )
        bear_transition = (
            not bear_confirmed
            and preflip_direction == "short"
            and preflip_status == "aligned"
            and shrink_pct >= self._to_float(cfg.get("transition_min_shrink_pct_short"), 0.55)
            and short_non_opposite
        )

        if bull_confirmed:
            score = max(
                self._to_float(cfg.get("confirmed_entry_score"), 0.84),
                self._to_float(signal.signal_score, 0.0),
            )
            if long_same_side:
            ...
            state.update(
                {
                    ...
                    "phase": "confirmed",
                    "side": "long",
                    "reason": f"4h_bull_confirmed:{signal_type_4h or direction_4h}",
                    "score": min(1.0, score),
                    "entry_allowed": bool(cfg.get("allow_confirmed_entry_without_15m", True))
                    and long_non_opposite
                    and vwap_score >= self._to_float(cfg.get("confirmed_min_vwap_score"), 0.08),
                    "entry_mode": "confirmed",
                    "entry_scale": self._to_float(cfg.get("confirmed_entry_scale"), 0.50),
                    "max_leverage": max(1, int(cfg.get("confirmed_max_leverage", 3))),
                    "min_vwap_score": self._to_float(cfg.get("confirmed_min_vwap_score"), 0.08),
                    "close_on_reverse": bool(cfg.get("close_on_reverse", True)),
                    "side_override_mode": "LONG_ONLY" if dynamic_side_override else "BOTH",
                }
            )
            return state
            ...
            state.update(
                {
                    ...
                    "phase": "confirmed",
                    "side": "short",
                    "reason": f"4h_bear_confirmed:{signal_type_4h or direction_4h}",
                    "score": min(1.0, score),
                    "entry_allowed": bool(cfg.get("allow_confirmed_entry_without_15m", True))
                    and short_non_opposite
                    and vwap_score >= self._to_float(cfg.get("confirmed_min_vwap_score"), 0.08),
                    ...
                    "min_vwap_score": self._to_float(cfg.get("confirmed_min_vwap_score"), 0.08),
                    "close_on_reverse": bool(cfg.get("close_on_reverse", True)),
                    "side_override_mode": "SHORT_ONLY" if dynamic_side_override else "BOTH",
                }
            )
            return state

        if bull_transition:
            score = self._to_float(cfg.get("transition_entry_score"), 0.72)
            if long_same_side:
                score += same_side_bonus
            state.update(
                {
                    ...
                    "phase": "transition",
                    "side": "long",
                    "reason": f"4h_bull_transition:aligned_shrink={shrink_pct:.2f}",
                    "score": min(1.0, score),
                    "entry_allowed": bool(cfg.get("allow_transition_entry", True))
                    and vwap_score >= self._to_float(cfg.get("transition_min_vwap_score_long"), 0.01),
                    "entry_mode": "transition",
                    "entry_scale": self._to_float(cfg.get("transition_entry_scale"), 0.35),
                    "max_leverage": max(1, int(cfg.get("transition_max_leverage", 2))),
                    "min_vwap_score": self._to_float(cfg.get("transition_min_vwap_score_long"), 0.01),
                    "close_on_reverse": bool(cfg.get("close_on_reverse", True)),
                    "side_override_mode": "LONG_ONLY" if dynamic_side_override else "BOTH",
                }
            )
            ...
            state.update(
                {
                    ...
                    "phase": "transition",
                    "side": "short",
                    "reason": f"4h_bear_transition:aligned_shrink={shrink_pct:.2f}",
                    "score": min(1.0, score),
                    "entry_allowed": bool(cfg.get("allow_transition_entry", True))
                    and vwap_score >= self._to_float(cfg.get("transition_min_vwap_score_short"), 0.08),
                    "entry_mode": "transition",
                    "entry_scale": self._to_float(cfg.get("transition_entry_scale"), 0.35),
                    "max_leverage": max(1, int(cfg.get("transition_max_leverage", 2))),
                    "min_vwap_score": self._to_float(cfg.get("transition_min_vwap_score_short"), 0.08),
                    "close_on_reverse": bool(cfg.get("close_on_reverse", True)),
                    "side_override_mode": "SHORT_ONLY" if dynamic_side_override else "BOTH",
                }
            )
    ...
    def _build_macd_v2_regime_entry_decision(
        ...
        *,
        ...
        if not regime_state.get("entry_allowed", False):
            return None

        side = str(regime_state.get("side", "neutral") or "neutral")
        if side not in {"long", "short"}:
            return None

        score = max(0.0, min(1.0, self._to_float(regime_state.get("score"), 0.0)))
        entry_scale = max(0.0, self._to_float(regime_state.get("entry_scale"), 1.0))
        is_trial_entry = str(regime_state.get("entry_mode", "")) == "transition"
        ...
        portion = max(0.0, min(1.0, portion * structure_scale))
        ...
        local_metadata["regime_fallback_allowed"] = True
        local_metadata["regime_fallback_score"] = float(score)
        ...
        local_metadata["session_risk"] = {
            "position_scale": float(session_position_scale),
            "state": str(signal.vwap_state or signal.signal_type_1h or ""),
        }
        local_metadata["vwap_structure_scale"] = {
            "state": str(signal.vwap_state or ""),
            "position_scale": float(structure_scale),
        }
        local_metadata["symbol_risk"] = {
    ...
    def _apply_symbol_side_override(
        ...
        runtime_override_mode: Optional[str] = None,
    ) -> FundFlowDecision:
        if decision.operation not in (Operation.BUY, Operation.SELL):
            return decision

        base_override_mode = self._get_symbol_side_override(decision.symbol)
        runtime_mode = self._normalize_symbol_side_override_mode(runtime_override_mode)
        override_mode = base_override_mode if base_override_mode != "BOTH" else runtime_mode
        ...
        metadata["symbol_side_override_base_mode"] = base_override_mode
        metadata["symbol_side_override_runtime_mode"] = runtime_mode
    ...
    @staticmethod
    def _direction_lock_allows_operation(direction_lock: Any, operation: Operation) -> bool:
        lock = str(direction_lock or "BOTH").strip().upper()
        if operation in (Operation.HOLD, Operation.CLOSE):
            return True
        if lock == "BOTH":
            return True
        if lock == "LONG_ONLY":
    ...
    def _enforce_direction_lock_on_decision(
        self,
        decision: FundFlowDecision,
        ...
        *,
        ...
        metadata = dict(decision.metadata or {})
        direction_lock = str((regime_info or {}).get("direction", metadata.get("direction_lock", "BOTH")) or "BOTH").upper()
        metadata["direction_lock"] = direction_lock
        metadata["direction_lock_hard_gate"] = True

        bypass_rule = self._match_direction_lock_bypass_rule(
            symbol=str(decision.symbol or ""),
            decision=decision,
            direction_lock=direction_lock,
        )
        if bypass_rule is not None:
            metadata["direction_lock_bypass_rule"] = dict(bypass_rule)
            metadata = self._annotate_primary_open_blocker(metadata)
            return replace(decision, metadata=metadata)

        if self._direction_lock_allows_operation(direction_lock, decision.operation):
            metadata = self._annotate_primary_open_blocker(metadata)
            return replace(decision, metadata=metadata)

        metadata["blocked_reason"] = f"direction_lock_hard_block:{direction_lock}"
        metadata["blocked_operation"] = decision.operation.value
        metadata["blocked_decision_reason"] = decision.reason
        metadata["direction_lock_source"] = source
        metadata["direction_lock_block"] = True
        metadata = self._annotate_primary_open_blocker(metadata)
        return FundFlowDecision(
            operation=Operation.HOLD,
            symbol=decision.symbol,
    ...
    def _match_direction_lock_bypass_rule(
        self,
        *,
        symbol: str,
        decision: FundFlowDecision,
        ...
        metadata = decision.metadata if isinstance(getattr(decision, "metadata", None), dict) else {}
        signal_type_norm = str(metadata.get("signal_type_1h", "") or "").strip().lower()
        vwap_state_norm = str(metadata.get("vwap_state", "") or "").strip().lower()
        signal_score = max(0.0, self._to_float(metadata.get("signal_score"), 0.0))
        vwap_score = max(0.0, self._to_float(metadata.get("vwap_score"), 0.0))
        stable_continuation_active = bool(metadata.get("stable_continuation_active", False))
        decision_side = "long" if decision.operation == Operation.BUY else "short" if decision.operation == Operation.SELL else ""
        direction_lock_norm = str(direction_lock or "").strip().upper()

        for raw_rule in self.direction_lock_bypass_rules:
            rule = dict(raw_rule)
            rule_direction_lock = str(rule.get("direction_lock", "") or "").strip().upper()
            rule_decision_side = str(rule.get("decision_side", "") or "").strip().lower()
            rule_signal_type = str(rule.get("signal_type_1h", "") or "").strip().lower()
            rule_vwap_state = str(rule.get("vwap_state", "") or "").strip().lower()
            rule_symbol = str(rule.get("symbol", "") or "").strip().upper()
            rule_symbols = (
                [str(item or "").strip().upper() for item in rule.get("symbols", []) if str(item or "").strip()]
                if isinstance(rule.get("symbols"), list)
                else []
            )
            min_signal_score = max(0.0, self._to_float(rule.get("min_signal_score"), 0.0))
            min_vwap_score = max(0.0, self._to_float(rule.get("min_vwap_score"), 0.0))
            require_stable_continuation_active = bool(rule.get("require_stable_continuation_active", False))

            if rule_direction_lock and rule_direction_lock != direction_lock_norm:
                continue
            if rule_decision_side and rule_decision_side != decision_side:
                continue
            if rule_signal_type and rule_signal_type != signal_type_norm:
                continue
            if rule_vwap_state and rule_vwap_state != vwap_state_norm:
                continue
            if rule_symbol and rule_symbol != symbol_norm:
                continue
            if rule_symbols and symbol_norm not in rule_symbols:
                continue
            if signal_score < min_signal_score:
                continue
            if vwap_score < min_vwap_score:
                continue
            if require_stable_continuation_active and not stable_continuation_active:
                continue
            return rule
        return None

    def _macd_v2_fast_exit_reason(
        self,
        *,
        position_side: str,
        ...
        if (not self.fast_exit_enabled) or position_side not in {"LONG", "SHORT"}:
            return ""
        ...
        if isinstance(timeframes, dict):
            tf_15m = timeframes.get("15m", {}) if isinstance(timeframes.get("15m"), dict) else {}
            tf_1h = timeframes.get("1h", {}) if isinstance(timeframes.get("1h"), dict) else {}

        direction_lock = str(
            regime_info.get("direction", market_flow_context.get("direction_lock", "BOTH")) or "BOTH"
        ).upper()
        confirmed_bars = int(
            self._to_float(
                market_flow_context.get(
                    "direction_lock_confirmed_bars",
                    tf_15m.get("direction_lock_confirmed_bars", tf_1h.get("direction_lock_confirmed_bars", 0)),
                ),
                0.0,
            )
        )
        lock_is_against = (
            (position_side == "LONG" and direction_lock == "SHORT_ONLY")
            or (position_side == "SHORT" and direction_lock == "LONG_ONLY")
        )
        if lock_is_against and confirmed_bars >= self.fast_exit_direction_lock_bars:
            return "direction_lock_reversed"

        if bool(market_flow_context.get("entry_signal_invalidated", False)):
            return "entry_signal_invalidated"

        sign = 1.0 if position_side == "LONG" else -1.0
        trap_score = self._to_float(
            market_flow_context.get("trap_score", tf_15m.get("trap_score", 0.0)),
            0.0,
        )
        cvd_momentum = self._to_float(
            market_flow_context.get("cvd_momentum", tf_15m.get("cvd_momentum", 0.0)),
            0.0,
        )
        oi_delta_ratio = self._to_float(
            market_flow_context.get("oi_delta_ratio", tf_1h.get("oi_delta_ratio", 0.0)),
            0.0,
        )
        imbalance = self._to_float(
            market_flow_context.get("imbalance", tf_15m.get("imbalance", 0.0)),
            0.0,
        )
        flow_reversal = (
            (cvd_momentum * sign < 0.0)
            or (oi_delta_ratio * sign < 0.0)
            or (imbalance * sign < -0.03)
        )
        if trap_score >= self.fast_exit_trap_high_confidence and flow_reversal:
    ...
    def _validate_macd_v2_entry_hard_gates(
        ...
        *,
        ...
        stable_continuation_active: bool = False,
        ...
        adx = self._to_float(tf_1h.get("adx", regime_info.get("adx")), 0.0)
        atr_pct = self._to_float(regime_info.get("atr_pct"), 0.0)
        if atr_pct <= 0 and price > 0:
            atr_pct = self._to_float(tf_1h.get("atr"), 0.0) / price
        spread_bps_raw = tf_15m.get("spread_bps", market_flow_context.get("spread_bps"))
        ...
        bypass_rule = self._match_entry_hard_gate_bypass_rule(
            symbol=symbol,
            direction=direction,
            signal_type_1h=signal_type_1h,
            vwap_state=vwap_state,
        ...
        if bypass_rule is not None:
            gate_metadata["entry_hard_gate_bypass_rule"] = dict(bypass_rule)

        l1_checks = {
            "adx_sufficient": adx >= self.entry_hard_gate_adx_min,
            "atr_in_range": self.entry_hard_gate_atr_min <= atr_pct <= self.entry_hard_gate_atr_max,
            "spread_ok": spread_ok,
        }
        if self.entry_hard_gate_require_regime_trend:
            l1_checks["regime_trend"] = regime == "TREND"
        l1_failed = [name for name, passed in l1_checks.items() if not passed]
        if l1_failed:
            allowed_l1_failures = {
                str(name or "").strip()
                for name in (bypass_rule or {}).get("allow_l1_failures", [])
                if str(name or "").strip()
            }
            if not allowed_l1_failures or not set(l1_failed).issubset(allowed_l1_failures):
                return False, f"L1_structure_failed:{','.join(l1_failed)}", gate_metadata
            gate_metadata["entry_hard_gate_bypassed_l1_failures"] = list(l1_failed)

        cvd_ratio = self._to_float(
            tf_15m.get("cvd_ratio", market_flow_context.get("cvd_ratio")),
            0.0,
        )
        oi_delta_ratio = self._to_float(
        ...
        gate_metadata["entry_hard_gate_flow_checks"] = dict(flow_checks)
        flow_score = sum(1 for passed in flow_checks.values() if passed)
        if flow_score < self.entry_hard_gate_flow_min_pass:
            return (
                False,
                "L2_flow_failed:"
                + ",".join(f"{name}={int(passed)}" for name, passed in flow_checks.items()),
                gate_metadata,
            )

        depth_ratio = self._to_optional_float(
            tf_15m.get("depth_ratio")
            if tf_15m.get("depth_ratio") is not None
            else market_flow_context.get("depth_ratio")
        )
        imbalance = self._to_optional_float(
            tf_15m.get("imbalance")
            if tf_15m.get("imbalance") is not None
            else market_flow_context.get("imbalance")
        )
        cvd_momentum = self._to_optional_float(
            tf_15m.get("cvd_momentum")
            if tf_15m.get("cvd_momentum") is not None
            else market_flow_context.get("cvd_momentum")
        )
        micro_checks: Dict[str, Optional[bool]] = {
            "depth_ok": None if depth_ratio is None else ((depth_ratio >= 1.02) if is_long else (depth_ratio <= 0.98)),
            "imbalance_ok": None if imbalance is None else ((imbalance >= 0.03) if is_long else (imbalance <= -0.03)),
            "cvd_momentum_ok": None if cvd_momentum is None else (cvd_momentum * sign > 0.0),
        }
        missing_micro_checks = [name for name, passed in micro_checks.items() if passed is None]
        if missing_micro_checks:
            gate_metadata["entry_hard_gate_missing_reason"] = (
                "L3_microstructure_missing:" + ",".join(missing_micro_checks)
            )
        available_micro_checks = {
            name: bool(passed)
            for name, passed in micro_checks.items()
            if passed is not None
        }
        gate_metadata["entry_hard_gate_micro_checks"] = dict(available_micro_checks)
        if not available_micro_checks:
            return True, "", gate_metadata

        required_micro_pass = min(self.entry_hard_gate_micro_min_pass, len(available_micro_checks))
        micro_score = sum(1 for passed in available_micro_checks.values() if passed)
        if micro_score < required_micro_pass:
            if bool((bypass_rule or {}).get("allow_microstructure_fail", False)):
                gate_metadata["entry_hard_gate_bypassed_microstructure"] = True
                return True, "", gate_metadata
            return (
                False,
                "L3_microstructure_failed:"
                + ",".join(f"{name}={int(passed)}" for name, passed in available_micro_checks.items()),
                gate_metadata,
            )

        return True, "", gate_metadata

    def _match_entry_hard_gate_bypass_rule(
        self,
        *,
        symbol: str,
        direction: str,
        ...
        signal_score_value = max(0.0, self._to_float(signal_score, 0.0))

        for raw_rule in self.entry_hard_gate_bypass_rules:
            rule = dict(raw_rule)
            ...
            if rule_symbols and symbol_norm not in rule_symbols:
                continue
            if signal_score_value < min_signal_score:
                continue
    ...
    def _resolve_pocket_management_override(
        self,
        *,
        signal_type_1h: str,
        vwap_state: str,
    ) -> Dict[str, Any]:
        if self.macd_v2_config is None:
            return {}
        return self.macd_v2_config.resolve_pocket_management_override(signal_type_1h, vwap_state)

    def _apply_pocket_leverage_constraints(
        self,
        *,
        leverage: int,
        pocket_management_override: Dict[str, Any],
        macd_v2_engine: Any,
        symbol: str,
    ) -> int:
        resolved = int(leverage)
        leverage_floor = max(0, int(self._to_float(pocket_management_override.get("leverage_floor"), 0)))
        if leverage_floor > 0:
            resolved = max(resolved, leverage_floor)
        leverage_cap = max(0, int(self._to_float(pocket_management_override.get("leverage_cap"), 0)))
        if leverage_cap > 0:
            resolved = min(resolved, leverage_cap)
        if (
            getattr(macd_v2_engine, "is_watchlist_symbol", None)
            and macd_v2_engine.is_watchlist_symbol(symbol)
            and int(getattr(macd_v2_engine.config, "symbol_risk_watchlist_max_leverage", 0) or 0) > 0
        ):
            resolved = min(
                resolved,
                int(getattr(macd_v2_engine.config, "symbol_risk_watchlist_max_leverage", 0) or 0),
            )
    ...
    def _build_tp_levels_from_override(
        ...
        *,
        ...
        pct_levels_raw = override.get("take_profit_pct_levels")
        reduce_levels_raw = override.get("take_profit_reduce_pct_levels")
        if not isinstance(pct_levels_raw, list) or not isinstance(reduce_levels_raw, list):
            return []

        pct_levels: List[float] = []
        for item in pct_levels_raw:
            pct = self._normalize_pct_ratio(item, 0.0)
            if pct > 0:
                pct_levels.append(pct)
        reduce_levels: List[float] = []
        for item in reduce_levels_raw:
            reduce_pct = max(0.0, min(1.0, self._to_float(item, 0.0)))
            if reduce_pct > 0:
                reduce_levels.append(reduce_pct)

        levels: List[Dict[str, float]] = []
        for pct, reduce_pct in zip(pct_levels, reduce_levels):
            if str(direction).lower() == "long":
                level_price = price * (1.0 + pct)
            else:
                level_price = price * (1.0 - pct)
            levels.append({"price": float(level_price), "reduce_pct": float(reduce_pct)})
        return levels
    ...
    def _score_lw(
        ...
        lw_score = self._to_float(combo_compare.get("score_macd_bb"), 0.0)
        components["combo_macd_bb"] = round(self._to_float(combo_compare.get("score_macd_bb"), 0.0), 3)
        components["combo_winner"] = -1.0
    ...
    def _score_ev(
        ...
        ev_score = self._to_float(ev_combo.get("score_macd_bb"), 0.0)
    ...
    def _detect_regime(self, market_flow_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            guide_direction in {"LONG_ONLY", "SHORT_ONLY"}
            and ev_dir in {"LONG_ONLY", "SHORT_ONLY"}
            and guide_direction != ev_dir
            and abs(float(guide_score)) <= float(self._direction_guide_conflict_neutral_zone)
            and not bool(price_action_override.get("applied", False))
        ):
            guide_direction = "BOTH"
            guide_score = 0.0
            guide_score_source = "ev_lw_conflict_neutralized"
    ...
    def _decide_macd_v2_strategy(
        ...
        for key in (
            "reject_reason_code",
            "reject_stage",
            "pocket_entry_override_label",
            "signal_score_threshold_used",
            "min_vwap_score_used",
            "min_entry_score_used",
            "direction_lock_applied",
            "entry_hard_filter_blocked",
            "entry_hard_filters",
            "entry_hard_gate_blocked",
            "entry_hard_gate_reason",
            "entry_hard_gate_checks",
            "entry_hard_gate_flow_checks",
            "entry_hard_gate_micro_checks",
        ):
            if key in signal_debug_details:
                metadata[key] = signal_debug_details.get(key)
        ...
        metadata["macd_4h_regime_state"] = regime_state
        metadata["regime_fallback_allowed"] = bool(regime_state.get("entry_allowed", False))
        metadata["regime_fallback_score"] = float(self._to_float(regime_state.get("score"), 0.0))
        metadata = self._annotate_primary_open_blocker(metadata)
        regime_runtime_mode = str(regime_state.get("side_override_mode", "BOTH") or "BOTH")
        regime_side = str(regime_state.get("side", "neutral") or "neutral")
        long_allowed_by_regime = regime_runtime_mode in {"BOTH", "LONG_ONLY"}
        short_allowed_by_regime = regime_runtime_mode in {"BOTH", "SHORT_ONLY"}
        take_profit_pct = self.take_profit_pct
        stop_loss_pct = self._normalize_pct_ratio(signal.stop_loss_pct, self.stop_loss_pct)

        fast_exit_reason = self._macd_v2_fast_exit_reason(
            position_side=pos_side,
            market_flow_context=market_flow_context,
            regime_info=regime_info,
        )
        if fast_exit_reason:
            metadata["fast_exit_triggered"] = True
            metadata["fast_exit_reason"] = fast_exit_reason
            return FundFlowDecision(
                operation=Operation.CLOSE,
                symbol=symbol,
                target_portion_of_balance=1.0,
                reason=f"fast_exit_{fast_exit_reason}",
                metadata=metadata,
            )
        ...
        if signal.direction == 'neutral':
            ...
            if (
                ...
                and regime_side in {"long", "short"}
                and (
                    (pos_side == "LONG" and regime_side == "short")
                    or (pos_side == "SHORT" and regime_side == "long")
                )
            ):
                metadata["regime_reverse_close"] = {
                    "position_side": pos_side,
                    "regime_side": regime_side,
                    "regime_state": regime_state.get("state"),
                    "regime_phase": regime_state.get("phase"),
                }
                return FundFlowDecision(
                    operation=Operation.CLOSE,
                    symbol=symbol,
                    target_portion_of_balance=1.0,
                    reason=f"macd_v2_regime_reverse_close_{pos_side.lower()}_{regime_state.get('state', 'neutral')}",
                    metadata=metadata,
                )
            ...
            close_signal_type = str(
                (current_pos or {}).get("signal_type_1h") or signal.signal_type_1h or ""
            )
            close_vwap_state = str(
                (current_pos or {}).get("vwap_state") or signal.vwap_state or ""
            )
            shrink_pocket_override = self._resolve_pocket_management_override(
                signal_type_1h=close_signal_type,
                vwap_state=close_vwap_state,
            )
            pocket_shrink_exit_enabled = bool(shrink_pocket_override.get("enable_4h_shrink_exit", True))
            if (
                pos_side
                and self.macd_v2_config
                and pocket_shrink_exit_enabled
                and getattr(self.macd_v2_config, "enable_4h_shrink_exit", False)
                ...
                shrink_loss_mitigation_active = bool(
                    getattr(self.macd_v2_config, "shrink_exit_loss_mitigation_enabled", False)
                    and pnl_ratio <= float(
                        getattr(self.macd_v2_config, "shrink_exit_loss_mitigation_pnl_threshold", -0.005)
                    )
                    and pnl_ratio <= float(
                        getattr(self.macd_v2_config, "shrink_exit_loss_mitigation_ignore_if_pnl_gt", 0.01)
                    )
                )
                if bool(shrink_pocket_override.get("disable_shrink_loss_mitigation", False)):
                    shrink_loss_mitigation_active = False
                if shrink_loss_mitigation_active:
                    close_ratio = min(
                        1.0,
                        max(
                            0.0,
                            float(getattr(self.macd_v2_config, "shrink_exit_loss_mitigation_exit_ratio", 0.60)),
                        ),
                    )
                    if close_ratio > 0.0:
                        ...
                        return FundFlowDecision(
                            ...
                            reason=f"macd_v2_4h_shrink_reduce_{pos_side.lower()}",
                            metadata=metadata,
                        )
                if getattr(self.macd_v2_config, "exit_4h_require_profit", True):
                    shrink_exit_ok = pnl_ratio > 0
                else:
                    shrink_exit_ok = pnl_ratio >= float(
                        getattr(self.macd_v2_config, "exit_4h_weak_loss_threshold", -0.02)
                    )
                if shrink_exit_ok:
                    metadata["shrink_exit"] = {
                        "direction": str(shrink_exit_policy.get("shrink_exit_direction", "") or ""),
        ...
        if signal.direction == 'long':
            if not long_allowed_by_regime:
                ...
                return FundFlowDecision(
                    operation=Operation.HOLD,
                    symbol=symbol,
                    reason=f"macd_v2_regime_block_long_{regime_state.get('state', 'neutral')}",
                    metadata=metadata,
                )
            # 检查是否有反向持仓需要平仓
            if pos_side == "SHORT":
            ...
            gates_passed, gate_reason, gate_metadata = self._validate_macd_v2_entry_hard_gates(
                symbol=symbol,
                direction="long",
                price=price,
                structural_vwap=structural_vwap,
            ...
            if not gates_passed:
                ...
                threshold_used = self._to_float(metadata.get("signal_score_threshold_used"), 0.0)
                metadata["entry_hard_gate_threshold_passed"] = bool(
                    threshold_used > 0 and signal.signal_score >= threshold_used
                )
                metadata["entry_hard_gate_failed_bucket"] = str(gate_reason or "").split(":", 1)[0]
                self._log_gate_check_report(
                    symbol, signal, metadata, regime_info, market_flow_context,
                ...
                return FundFlowDecision(
                    operation=Operation.HOLD,
                    symbol=symbol,
                    reason=f"entry_hard_gate_block:{gate_reason}",
                    metadata=metadata,
                )

            direction_1h_raw = str(
                (signal.details or {}).get("direction_1h", tf_1h.get("direction", ""))
                or ""
            ).strip().lower()
            if direction_1h_raw in {"long", "bullish", "bull", "up"}:
                bar_1h_direction = "BULLISH"
            elif direction_1h_raw in {"weakly_bullish", "weak_bullish"}:
                bar_1h_direction = "WEAKLY_BULLISH"
            elif direction_1h_raw in {"short", "bearish", "bear", "down"}:
                bar_1h_direction = "BEARISH"
            elif direction_1h_raw in {"weakly_bearish", "weak_bearish"}:
                bar_1h_direction = "WEAKLY_BEARISH"
            else:
            ...
            pocket_management_override = self._resolve_pocket_management_override(
                ...
                vwap_state=str(signal.vwap_state or ""),
            )
            leverage = macd_v2_engine.calculate_leverage(
                signal.signal_score,
                signal.ema_multiplier,
            ...
            leverage = self._apply_pocket_leverage_constraints(
                leverage=leverage,
                pocket_management_override=pocket_management_override,
                macd_v2_engine=macd_v2_engine,
                symbol=symbol,
            )
            # 三共振动态杠杆调整 (新增)
            ...
            portion = max(0.0, min(1.0, portion * structure_scale))
            pocket_position_scale = self._to_float(pocket_management_override.get("position_scale"), 1.0)
            if pocket_position_scale > 0:
                portion = max(0.0, min(1.0, portion * pocket_position_scale))
            pocket_max_target_portion = self._normalize_portion_ratio(
                pocket_management_override.get("max_target_portion"),
                0.0,
            )
            if pocket_max_target_portion > 0:
                portion = min(portion, pocket_max_target_portion)
            metadata["session_risk"] = {
                "position_scale": float(session_position_scale),
                "state": str(signal.vwap_state or signal.signal_type_1h or ""),
            }
            metadata["vwap_structure_scale"] = {
                "state": str(signal.vwap_state or ""),
                "position_scale": float(structure_scale),
            }
            ...
            suggested_stop = self._to_optional_float(signal.suggested_stop_price)
            stop_loss_price = suggested_stop
            pocket_stop_loss_pct = self._normalize_pct_ratio(
                pocket_management_override.get("sl_pct"),
                stop_loss_pct,
            ) if pocket_management_override else stop_loss_pct
            if stop_loss_price is None or stop_loss_price <= 0 or stop_loss_price >= price:
                stop_loss_price = price * (1.0 - pocket_stop_loss_pct) if pocket_stop_loss_pct > 0 else None
            pocket_take_profit_pct = self._normalize_pct_ratio(
                pocket_management_override.get("tp_pct"),
                take_profit_pct,
            ) if pocket_management_override else take_profit_pct
            take_profit_price = price * (1.0 + pocket_take_profit_pct) if pocket_take_profit_pct > 0 else None
            tp_levels = self._build_tp_levels_from_override(
                price=price,
                direction="long",
                override=pocket_management_override,
            )
            metadata["tp_sl"] = {
                "tp_pct": pocket_take_profit_pct,
                "sl_pct": pocket_stop_loss_pct,
                "tp_enabled": take_profit_price is not None,
                "sl_enabled": stop_loss_price is not None,
            }
            if pocket_management_override:
                metadata["pocket_management_override"] = dict(pocket_management_override)
            if tp_levels:
            ...
            if not short_allowed_by_regime:
                metadata["regime_block"] = {
                    "blocked_signal_direction": "short",
                    "regime_state": regime_state.get("state"),
                    "regime_phase": regime_state.get("phase"),
                    "regime_runtime_mode": regime_runtime_mode,
                }
                return FundFlowDecision(
                    operation=Operation.HOLD,
                    symbol=symbol,
                    reason=f"macd_v2_regime_block_short_{regime_state.get('state', 'neutral')}",
                    metadata=metadata,
                )
            # 检查是否有反向持仓需要平仓
            if pos_side == "LONG":
            ...
            if direction_1h_raw in {"short", "bearish", "bear", "down"}:
                bar_1h_direction = "BEARISH"
            elif direction_1h_raw in {"weakly_bearish", "weak_bearish"}:
                bar_1h_direction = "WEAKLY_BEARISH"
            elif direction_1h_raw in {"long", "bullish", "bull", "up"}:
                bar_1h_direction = "BULLISH"
            elif direction_1h_raw in {"weakly_bullish", "weak_bullish"}:
                bar_1h_direction = "WEAKLY_BULLISH"
            else:
    ...
    def _decide_macd_strategy(
        ...
        if not tf_15m or not tf_1h or not tf_4h:
            return FundFlowDecision(
                ...
                metadata={
                    "error": f"missing 15m/1h/4h data",
                    "strategy_mode": "macd_mtf_strategy",
                    "macd_tf_diagnostics": tf_diagnostics,
                }
            )
    ...
    def decide(
        ...
        direction = str(regime_info.get("direction", "BOTH")).upper()
        if self.rule_strategy_enabled:
            decision = self._decide_rule_strategy(symbol, portfolio, price, market_flow_context or {}, regime_info)
            return self._enforce_direction_lock_on_decision(decision, regime_info, source="rule_strategy")
        ...
        if self.macd_mtf_strategy_enabled:
            ...
            return self._enforce_direction_lock_on_decision(decision, regime_info, source="macd_mtf")
