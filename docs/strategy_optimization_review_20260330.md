...
def _check_fast_exit_conditions(self, symbol, position, context) -> tuple[bool, str]:
    ...
    if context.get("trap_score", 0) >= 0.85:          # 注意：提高阈值到 0.85，草稿 0.7 偏低
    ...
    trap_moderate = context.get("trap_score", 0) >= 0.7   # 中等 trap
    lock_soft     = lock_is_against and confirmed_bars >= 1  # 方向锁反向但未满足 L1 确认数
...
2/3 既能提前捕捉方向恶化，又比 3/3 减少 1~2 根 K 线的延迟。  
...
def _check_time_based_exit(self, symbol, position, context) -> tuple[bool, str]:
    ...
    ENTRY_TYPE_PARAMS = {
        ...
        "unknown":            {"observe_min": 30, "min_profit_pct": 0.0035},  # 保守默认
    }
...
主要问题是：**trailing stop distance 0.5% 在 ATR 窗口（0.6%~1.8%）里过小**，会在正常波动中频繁触发，把趋势仓过早锁死。
...
def _calculate_trailing_distance(self, symbol, context) -> float:
    ...
    """
    atr_pct = context.get("atr_pct", 0.010)      # 当前 ATR（价格百分比）
...
class PartialTPState:
    ...
    def __init__(self):
        self.levels_completed: list[int] = []   # 已完成的 level index
        self.trailing_activated: bool = False
        self.trailing_high_water: float = 0.0   # 追踪止盈的最高价（多头）或最低价（空头）
...
def _check_partial_tp(self, symbol, position, context) -> DecisionResult | None:
    ...
    sl_pct = self.config.stop_loss_pct     # 0.012
    unrealized_pct = position.unrealized_pnl_pct
    sign = 1 if position.side == "long" else -1
    ...
    for i, level in enumerate(self.config.partial_tp_levels):
        ...
        trigger_profit = sl_pct * level["trigger_r_multiple"]   # e.g. 1R = 0.012, 2R = 0.024
...
def _update_trailing_stop(self, symbol, position, context) -> None:
    ...
    state: PartialTPState = self._get_or_create_tp_state(symbol)
    activation_pct = self.config.trailing_stop_activation_pct   # 0.012
    unrealized_pct = position.unrealized_pnl_pct
    sign = 1 if position.side == "long" else -1
    ...
    # 通知 protection 层更新 SL 价格（而不是在这里直接平仓）
...
def _validate_entry_hard_gates(
    ...
    layer1 = {
        ...
        "atr_in_range":   0.006 <= context.get("atr_pct", 0) <= 0.020,  # 上限放宽到 2%
        "spread_ok":      context.get("spread_bps", 999) <= 0.0008,     # 放宽到 0.8 bps
    }
    ...
    # 第三项：CVD momentum 方向（与 layer2 的 cvd_ratio 不同维度）
...
这三者本身就有相关性（同一方向的流向信号），且在趋势刚建立时 VWAP 偏移本就有延迟。全部 AND 会在趋势初期系统性错过入场机会。2/3 既保留了多维流向验证，又允许一个维度滞后。
...
+ "entry_hard_gate_flow_min_pass": 2,
  // CVD ratio、OI delta、VWAP 对齐

  // 第三层：盘口层（2/3 通过）
+ "entry_hard_gate_micro_min_pass": 2,
  // depth ratio、imbalance、CVD momentum
```
